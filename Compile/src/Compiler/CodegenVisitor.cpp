...
struct LiteralCodeGenerator : boost::static_visitor<Constant *>
{
    ...
    Constant *operator ()(float const& value) const
    {
...
void CExpressionCodeGenerator::Visit(CBinaryExpressionAST &expr)
{
	...
	*/
	...
	PValue a = m_values.at(m_values.size() - 2);
	PValue b = m_values.at(m_values.size() - 1);
...
void CExpressionCodeGenerator::Visit(CCallAST &expr)
{
	...
	for (auto & arg : args)
	{
		addArgs.push_back(arg.get());
	}

	PValue pValue(m_builder.CreateCall(pFunction, addArgs, "calltmp"));
	if (pValue->getType()->isPointerTy())
	{
		m_context.GetExpressionStrings().Manage(pValue.get());
	}
	m_values.push_back(pValue);
}
...
PValue CExpressionCodeGenerator::GenerateNumericExpr(PValue a, BinaryOperation op, PValue b)
{
	...
	bool leftIsFloat = (leftType == llvm::Type::FloatTyID);
	bool rightIsInt = (rightType == llvm::Type::IntegerTyID);
	bool rightIsFloat = (rightType == llvm::Type::FloatTyID);
...
PValue CExpressionCodeGenerator::GenerateStringExpr(PValue a, BinaryOperation op, PValue b)
{
    switch (op)
    {
    case BinaryOperation::Add:
